...
PHP_ARG_ENABLE(cheetah, whether to enable cheetah support,
Make sure that the comment is aligned:
[  --enable-cheetah           Enable cheetah support])
...
if test "$PHP_CHEETAH" != "no"; then
  ...
  dnl [
  dnl   PHP_ADD_LIBRARY_WITH_PATH($LIBNAME, $CHEETAH_DIR/$PHP_LIBDIR, CHEETAH_SHARED_LIBADD)
  dnl   AC_DEFINE(HAVE_CHEETAHLIB,1,[ ])
  dnl ],[
  dnl   AC_MSG_ERROR([wrong cheetah lib version or lib not found])
  dnl ],[
  dnl   -L$CHEETAH_DIR/$PHP_LIBDIR -lm
  dnl ])
  dnl
  dnl PHP_SUBST(CHEETAH_SHARED_LIBADD)

  PHP_NEW_EXTENSION(cheetah, cheetah.c, $ext_shared,, -DZEND_ENABLE_STATIC_TSRMLS_CACHE=1)
fi
